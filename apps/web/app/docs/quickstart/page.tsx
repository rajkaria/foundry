import { DocsLayout, H2, P, Lead, Code, CodeBlock, Callout, PageNav } from "@/components/docs/DocsLayout";

export const metadata = {
  title: "Quickstart — Foundry docs",
  description: "From npm install to a paid inference call against a Foundry Ingot in three minutes.",
};
...
export default function QuickstartPage() {
  return (
    <DocsLayout
      ...
      intro={
        <Lead>
          By the end of this page, you will have called a Foundry Ingot,
          received a response, and seen the inference and revenue transaction
          hashes that proved the call paid out to the model's co-owners.
          Target time: three minutes on a clean machine.
        </Lead>
      }
      toc={toc}
    >
      <H2 id="install">1 · Install</H2>
      <P>The SDK has one runtime dependency (<Code>viem</Code>) and is fully tree-shakeable.</P>
...
# or: yarn add @foundryprotocol/sdk`}</CodeBlock>
      ...
      <P>
        Three lines. The first creates the client; the second fires the
        inference call; the third logs the receipt.
      </P>
      <CodeBlock lang="ts" filename="hello-foundry.ts">{`import { Foundry } from "@foundryprotocol/sdk";
...
console.log(receipt);`}</CodeBlock>
      ...
      <Callout tone="ember" title="No wallet required to read">
        <p>
          Inference calls go through the OpenAI-compatible HTTP proxy.
          You do <em>not</em> need a wallet to call an Ingot — revenue
          accrues to its co-owners regardless of who initiated the call.
          You only need a wallet when you want to <strong>claim</strong>
  ...
  receipt: {
    ...
    latencyMs:        842,
      ...
      <P>
        Both tx hashes land on 0G Aristotle within ~4 seconds. The Forge in
        Public dashboard reflects them in real time.
      </P>
      ...
      <ul className="text-body-lg space-y-3 text-platinum-300 ml-6 list-disc">
        <li>
          ...
          <a href="/docs/adapters" className="text-ember-400 hover:text-ember-300">Adapters</a> —
          Vercel AI SDK, LangChain, and the OpenAI-compatible HTTP proxy.
        </li>
        <li>
          Want to contribute data to a Forge and earn shares? See{" "}
          <a href="/docs/build-on-foundry" className="text-ember-400 hover:text-ember-300">Build on Foundry</a>.
        </li>
        <li>
          Want to understand how shares are computed? See{" "}
          <a href="/docs/attribution" className="text-ember-400 hover:text-ember-300">Verifiable attribution</a>.
        </li>
      </ul>
