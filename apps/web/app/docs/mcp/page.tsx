...
export default function McpPage() {
  return (
    <DocsLayout
      ...
      intro={
        <Lead>
          <Code>@foundryprotocol/mcp</Code> is a Model Context Protocol server
          that exposes Foundry Ingots — the co-owned, revenue-routing AI models
          on 0G — to any MCP-capable agent. Wire it into Claude Desktop, Cursor,
          Cline, or your own MCP client and your agent gains five new tools
          backed by live mainnet Ingots.
        </Lead>
      }
      toc={toc}
    >
      ...
      <P>
        The SDK and adapters cover the case where you&apos;re writing code that
        calls a model. MCP covers the other case — when an autonomous agent (a
        Claude session, a Cursor copilot, a Cline workflow) needs to discover
        and invoke models on its own. The same OpenAI-compat proxy backs both
        paths, so the inference, attestation, and on-chain revenue settlement
      ...
      <Callout tone="ember" title="The agent does not need to know it&apos;s on 0G">
        <p>
          ...
          <Code>run_inference</Code>, <Code>get_ingot</Code>,{" "}
          <Code>get_lineage</Code>, <Code>get_attestation</Code>. It reasons
          about models the way it reasons about any other tool. The fact that
          each call reserves a fee on 0G Chain and credits an Ingot&apos;s
          co-owners is invisible to the agent and verifiable by anyone.
        </p>
      </Callout>
...
pnpm add @foundryprotocol/mcp`}</CodeBlock>
      <P>Published on npm at <Code>@foundryprotocol/mcp</Code>. MIT-licensed.</P>
  ...
  "mcpServers": {
    "foundry": {
      ...
      <P>
        Restart Claude Desktop. The five Foundry tools appear in the tools
        list. Ask Claude to <em>&ldquo;list the Foundry Ingots and translate
        &lsquo;how was your weekend?&rsquo; to Konkani using the best one&rdquo;</em> and
        it will chain <Code>list_ingots</Code> → <Code>run_inference</Code>{" "}
        without further prompting.
      </P>

      <H2 id="cursor">Cursor</H2>
      <P>
        In Cursor&apos;s settings, add an MCP server entry pointing to the
        same command. Cursor&apos;s agent mode will discover the tools
        automatically.
      </P>
    ...
    "foundry": {
      ...
      <Table
        ...
        rows={[
          ...
          [
            <Code>get_ingot</Code>,
            "Metadata + cap table for a specific Ingot.",
          ],
          [
  ...
  "temperature":  0.6
      ...
      <P>
        The response carries the model output plus a <Code>foundry</Code>{" "}
        object: the resolved Ingot id, the mode flag (<Code>live</Code> /{" "}
        <Code>tee</Code> / <Code>stub</Code>), and the on-chain tx hashes for
        inference and revenue split when fees were reserved.
      </P>

      <H2 id="programmatic">Programmatic use</H2>
      <P>
        Wire the server into a custom MCP client (or write your own MCP
        bridge) using the exported factory:
      </P>
      <CodeBlock lang="ts" filename="custom-mcp.ts">{`import { createFoundryMcpServer } from "@foundryprotocol/mcp";
...
await server.connect(new StdioServerTransport());`}</CodeBlock>
      ...
      <ol className="text-platinum-300 mt-2 list-decimal space-y-1 pl-5">
        ...
        <li>
          ...
          <Code>RevenueSplitter.receivePayment(tokenId)</Code> — the
          per-Ingot pull-payment ledger.
        </li>
        <li>
          Co-owners pull-claim their pro-rata share at any time. The Ingot
          page and the dashboard reflect the new claimable balance within
          seconds.
        </li>
      </ol>
      <P>
        This is the same settlement path used by the SDK, the Vercel AI / LangChain
        adapters, and direct cURL calls. The MCP server is a new front door, not a
        new payment rail.
      </P>
