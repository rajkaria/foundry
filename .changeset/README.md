...
for the neutral `@foundryprotocol/0gkit-*` packages (and `create-foundry-app`).
...
The `@foundryprotocol/0gkit-*` packages are **linked**: a bump to any one moves them all to the
same version, so a builder never has to reason about a cross-package version
matrix. Private apps (`@foundryprotocol/0gkit-playground`, the web app) are not published and
