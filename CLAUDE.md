...
3. **K6 follow-ups (Minor, triage later, from the final review):** (a) lazy-dep convention now split — `0gkit-mcp` is a runtime dep of the CLI but `0gkit-kits`/`0gkit-jobs` are still devDeps that a global `0g add`/`0g kits`/`0g jobs` can't resolve; reconcile (make them deps too, or document). (b) durable-agent's `mcpToolPlugin(_env)` ignores its arg (reads `process.env` via module singletons — documented, honest, but asymmetric with the other 3 kits). (c) **DONE in K7** — `check-kits.mjs` overlay staging fixed to the tier-prefixed layout (`kits:check` 26 FAIL → 27 PASS).
